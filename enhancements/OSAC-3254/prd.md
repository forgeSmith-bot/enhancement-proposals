...
- **Boot MAC Address:** The media access control (MAC) address of the primary physical network interface card (NIC) used by the physical host during its Preboot Execution Environment (PXE) boot process to initiate network boot and register with the fulfillment service.
- **Primary IP Address:** The primary IPv4 or IPv6 address assigned to the host from the backend inventory that is reachable on the tenant's VirtualNetwork, enabling self-service Tenant User SSH connections.
...
- **Technical Documentation:** Updating the user-facing API reference, CLI reference guides (including help commands and man pages), and the Tenant User Getting Started Guide to cover the newly exposed boot MAC address and primary IP address fields in the BareMetalInstance status.
...
- As a Cloud Infrastructure Admin, I want an automated synchronization mechanism to keep the BareMetalInstance network metadata updated with backend inventory changes without manual user intervention, ensuring the metadata remains current and accurate across the fleet.
...
- [ ] **Metadata Synchronization:** The automated synchronization mechanism successfully updates the BareMetalInstance status with current backend inventory network metadata without requiring manual user intervention, ensuring metadata remains current and accurate.
...
- **Existing Synchronization Capability:** The OSAC platform already possesses an existing capability for administrators to trigger a status refresh of a BareMetalInstance, which will be leveraged to update metadata without introducing new user-facing actions or APIs.
...
- **BareMetalInstance Status Refresh:** The platform's status refresh capability must support updating stale metadata for BareMetalInstances to ensure they reflect current backend inventory states.
- **Tenant Network Routing:** The tenant's VirtualNetwork infrastructure must support routing and connectivity to the host's primary IP address.
  - *Validation Criteria:* Direct IP reachability must be verified through automated tests executing SSH connections from within the tenant namespace to the provisioned BareMetalInstance.
...
- **Metadata Drift Post-Propagation:**
  ...
  - *Impact / Mitigation:* This is mitigated because the metadata reflects the state at provisioning time, and administrators can trigger a status refresh or reboot the instance to update the status to match current inventory records.
...
- **Unreachable Primary IP Address on Tenant Network:**
  - *Risk:* The primary IP address surfaced from the backend inventory is only reachable on an infrastructure-level network, preventing Tenant Users from connecting via their VirtualNetwork.
  - *Impact / Mitigation:* This is mitigated by ensuring that only IP addresses that are routable over the tenant's VirtualNetwork are selected for propagation, and by validating network path routing during pre-provisioning integration checks.