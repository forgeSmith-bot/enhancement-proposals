...
- **Automatic Metadata Propagation:** The bare metal fulfillment service must retrieve the physical host's boot MAC address and primary IP address from the inventory backend upon host assignment.
...
- **Bare Metal Inventory Service API:** The inventory service must expose the physical host's boot MAC address and IP address via its existing query APIs so the operator can retrieve them during fulfillment.
- **CaaS / Assisted Installer Integration:** The CaaS layer must support querying the BareMetalInstance status API to read the MAC address and integrate it into its agent mapping controller.